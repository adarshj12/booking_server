...
const login = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user._id, name: user.username, mobile: user.mobile, user: true }, process.env.SECRET,{ expiresIn: "10h" });
        res.status(202)
...
const googleAuth = async (req, res) => {
    try {
        ...
        if (!userExist) {
            ...
            await newUser.save();
            const token = jwt.sign({ id: newUser._id, name: newUser.username, user: true }, process.env.SECRET,{ expiresIn: "10h" });
